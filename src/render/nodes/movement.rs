...
use crate::render::{SdlWrapper, Sprite, TextureMap};
use crate::utils::{Direction, DirectionMap, Vector2};
...
pub fn render<'r>(
	...
	if let Some(path) = &game.nodes.path {
		let frame = (app.frame / game.framerate) as i32;
		if path.len() != 0 {
		...
		for i in 0..path.len() {
			...
			let sprite = match (last, next) {
				...
				(Some(last), Some(next)) => draw_path_mid(&node, last, next),
				(Some(last), None) => draw_path_start(&node, last),
				(None, None) => continue,
			};
...
fn draw_path_mid(position: &Vector2, last: &Vector2, next: &Vector2) -> Option<RenderSprite> {
...
fn draw_path_start(position: &Vector2, last: &Vector2) -> Option<RenderSprite> {
